...
export default function GlitterWarp({
  ...
  }, [speed, color, density, brightness, starSize, focalDepth, turbulence, autoPlay])
  ...
  return (
    <div
      className={cn('glitter-warp', className)}
      style={{
        width: widthStyle,
        height: heightStyle,
      }}
    >
      <div ref={containerRef} className="glitter-warp__canvas" aria-hidden="true" />
      {children ? <div className="glitter-warp__content">{children}</div> : null}
    </div>
  )
}
